...
Every commit subject must match `<type>(<scope>): <message>`. Allowed types are `feat`, `fix`, `docs`, `test`, `refactor`, `perf`, `build`, `ci`, `chore`, and `revert`. Use a meaningful lowercase kebab-case scope owned by the change, such as `translation`, `workflow`, `api`, `web`, `media`, `security`, `delivery`, or `governance`. Write the message in concise imperative form, start it with lowercase, and omit the trailing period. For example: `feat(translation): add tone preset selection`. Put rationale, migration notes, issue references, and `BREAKING CHANGE:` details in the body or footer. A missing scope, an unlisted type, or subjects such as `update files` do not satisfy the repository convention.
