...
The implemented Phase 2 CI runs Python quality plus real PostgreSQL/Garage migration contracts, web quality/build, and Compose validation. The Python job pins Ubuntu 24.04 and installs an explicit FFmpeg package version before running the mandatory media fixture; hosted-runner software must never be assumed implicitly. Only a protected-main push that passes all three job groups may publish application images. Pull requests never receive `packages: write`, never publish images, and never run on the production host.
